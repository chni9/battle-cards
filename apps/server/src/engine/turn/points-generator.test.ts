...
describe('Points Generator (L5-08)', () => {
  it('gains on the user turn including the play turn; stops when counter is gone', () => {
    ...
    // Paid 5, then step 4 grants +3 on the play turn.
  ...
  it('grants 6 points per turn when upgraded (L58-04)', () => {
    const state = createInitialState({
      seats: [
        { id: 'a', nickname: 'A' },
        { id: 'b', nickname: 'B' },
    ...
    a.specialCards = [{ instanceId: 'pg-1', cardId: 'points-generator', isUpgraded: true }];
    a.points = 5;
    a.pendingEffects = [];
    b.pendingEffects = [];
