...
| **#V5-3** | Weaker difficulty tiers: keep uniform random substitution, or softmax over search action values? | **Closed (L36-03):** Softmax for Normal; Easy = sync `heuristic-v4` + existing uniform rates; Hard = full search, no substitution. | — |
| **#V5-4** | Does the public `Why` panel expose search statistics (visits, estimated win probability)? | **Closed (L36-04):** No numbers. Codes `search-best` / `search-fallback` only. `PROTOCOL_VERSION` 28. | — |
