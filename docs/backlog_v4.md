...
| ID | Task | Cx | Risk | Depends on | Status |
|---|---|---|---|---|---|
| L21-01 | **Blocked on #V4-29.** Extend the 20-point random special purchase to draw from all 20 specials — the 6-card restriction was ruling §6.2 #10, a scope artefact. **Acceptance:** the draw uses the injected `rng` over `SPECIAL_CARD_IDS`; a test proves a pending-handler special is never granted by this path, or is granted and unplayable, per the design chosen in L20-04. | S | Medium | L20-04 | Blocked |
| L21-02 | **Blocked on #V4-17, #V4-18, #V4-33.** `super-regeneration` (+9 / +18 lives through `gainLives` with `GameState.lifeLimit`) and `upgrade-point-thief` (mass UP theft + `downgradeAllCards`, 1 UP per upgrade removed; upgraded also steals all points). **Acceptance:** tests for base and upgraded; a test proves an upgrade removed from a kit-trait-upgraded copy yields 1 UP and that a copy acquired *afterwards* arrives upgraded again; the ruled Counter Rule scope is tested. | M | Medium | L20-11, L20-04 | Blocked |
| L21-03 | **Blocked on #V4-19, #V4-33, #V4-34, #V4-35.** `card-thief`: steals a random card from a chosen opponent, a chosen card if that opponent is spied per the #V4-35 ruling; upgraded steals from every opponent. **Do not assume §10.1 parity holds** — it depends on both #V4-34 (empty hand) and #V4-35 (spied by whom); see technical spec v4 §10.1. **Acceptance:** tests for base, upgraded and the spied branch; `list-legal-actions-view-guard.test.ts` passes; the stolen card's identity reaches the thief and the victim only, never the public action log. | M | Medium | L20-10, L20-18, L20-04 | Blocked |
...
| ID | Task | Cx | Risk | Depends on | Status |
|---|---|---|---|---|---|
| L23-01 | **Blocked on #V4-1, #V4-3, #V4-32.** `mega-attack`: one pending attack per target through the multi-target pattern, shield applies via `applyDamage`. Redirectability goes in `listEligibleMirrorTargets`: base MEGA only by an **upgraded** Mirror, upgraded MEGA never — neither falls out of the existing filter, which reads the *attack's* upgrade state. Whether the Assassin may include it in a multi-attack is #V4-32 and must **not** be decided in this task. **Acceptance:** MEGA appears in no shop list and no starting deal (both implied by rules spec §5); its multi-attack eligibility matches the #V4-32 ruling; tests cover 2, 3 and 4 players, the shield path, both redirection rules, and two MEGAs facing each other. | **L** | **High** | L20-05, L20-07, L20-15 | Blocked |
| L23-02 | **Blocked on #V4-4.** `super-mirror`: every attack pending on the user is redirected to every opponent, each independently, through `duplicatePendingEffect` with fresh ids and `redirectedBy: 'super-mirror'`. Upgraded doubles the damage via `damageMultiplier`. Redirected effects are ineligible for a regular Mirror and eligible for another Super Mirror. **Acceptance:** 2 pending attacks against a user with 3 opponents produce 6 distinct pending effects with 6 distinct ids; a test proves a regular Mirror cannot pick them up and a Super Mirror can. **Watch point:** the doubling multiplies collisions in the mutual-attack comparison — L20-07's tests must still pass. | **L** | **High** | L20-15, L23-01 | Blocked |
| L23-03 | **Blocked on #V4-5, #V4-31, #V4-33.** `attack-thief`: one charge blocking a single attack targeting the user, plus a random attack card stolen from each opponent (all attack cards from all opponents when upgraded). The charge must **not** be stored in `PersistentEffect.counter`, which `applyDamage` decrements per life lost. Whether MEGA ATTACK is stealable is #V4-31 and must **not** be decided in this task. **Acceptance:** a test proves the charge survives an attack cancelled by mutual attacks and is spent on the next one, per the #V4-5 ruling; the steal filter matches the #V4-31 ruling; the charge's presence is public and its count is not. | M | **High** | L20-05, L20-10, L20-12, L20-04 | Blocked |
...
| ID | Task | Cx | Risk | Depends on | Status |
|---|---|---|---|---|---|
| L26-01 | **Blocked on #V4-11, #V4-12, #V4-36.** `reanimation` base: armed on play; on a later elimination the user returns with a random kit and the resources the #V4-36 ruling specifies. Intervention point is `processEliminations`, **before** `captureEliminationSnapshot` and **before** `cleanupEliminatedPlayer` — the latter already pools `activePersistentEffects` and empties them, irreversibly. Extract a reusable player-reset from `create-initial-state.ts`'s private `makePlayer`, and note `makePlayer` deals resources **and** cards **and** kit specials, so what the reset includes is exactly what #V4-36 rules on — do not pick silently. The fate of the player's existing hand is part of the same ruling. Consolidate the three duplicated sole-survivor checks (`findSoleSurvivorId` and the two `findWinner`s) so game-over cannot fire on a player with an armed Reanimation. Do not call `rejectReconnection` for a reanimated player. **Acceptance:** §10.3's suite passes — re-entry into turn order, no premature game-over, a **fresh** `eliminationSnapshot` on the second death (`captureEliminationSnapshot` early-returns today), reconnection preserved. **Watch point:** `isEliminated` is read at 77 non-test sites across 35 files and has never been reset. Read the produced code line by line. | **L** | **High** | L20-18, L20-04 | Blocked |
| L26-02 | **Blocked on #V4-13.** Upgraded `reanimation`: the user chooses their kit. This sub-choice is raised **inside `processEliminations`**, possibly outside the choosing player's turn, which makes `processEliminations` interruptible — strictly harder than both existing sub-choices. **Acceptance:** turn advance and game-over stay paused while the choice is open; expiry falls back to a seeded random kit; a bot resolves it. | **L** | **High** | L26-01 | Blocked |
...
| L27-03 | `indestructible`: 18 lives, 1 attack card, Tax and Regeneration always upgraded. **Acceptance:** catalog entry verified value by value; a test proves Tax bought mid-game arrives upgraded. | S | Low | L21-02 | To do |
...
| L27-07 | `witch`: 1 UP, Thief always upgraded, Reanimation + Poison. **Acceptance:** catalog entry verified; both specials playable. | S | Low | L22-01, L26-01 | To do |
| L27-08 | `wizard`: draw 2, Thief always upgraded, MEGA ATTACK. Art maps to `Magician.png` — the id-to-filename map is declared, not derived. **Acceptance:** catalog entry verified; the art entry resolves. | S | Low | L23-01 | To do |
...
| ID | Task | Cx | Risk | Depends on | Status |
|---|---|---|---|---|---|
| L28-01 | **Blocked on #V4-22.** `ghost`: 14 lives, Curse, and 2 points per life lost from any cause. The credit happens **caller-side** from the outcome the life primitives return — never inside them (golden rule 2 forbids enriching either). Gains are on lives *actually* lost, after shield absorption. **Acceptance:** tests cover damage through a shield, Tax, Imposition's ceded life, Poison and Suicide; the ruled treatment of Sentence, self-elimination and Cloning is tested explicitly. | M | **High** | L20-08, L20-09, L22-02 | Blocked |
| L28-02 | **Blocked on #V4-23 and #V4-24.** `duplicator`: 2 lives, 1 action card, 0 attack cards, Imposition + Attack Thief, and activatable duplication as a **new `TurnAction` variant** replacing the turn's action. Anticipatory, covers the following table round only, renewed each turn. Reads `origin: 'direct'` on the gain primitives so two Duplicators do not loop. Life gains clamp at `GameState.lifeLimit`. Same new-action-type pattern as L25-02 — reuse it. **Acceptance:** the new action type is handled in all 10+ contact points including the bot's `scoreAction` and the action-log `switch`; tests cover activation, expiry after one table round, non-renewal, two Duplicators in one game, and a gain from theft and from an elimination reward. **Watch point:** produce `Duplicator.png` — the only missing asset in the whole of V4, and a missing PNG compiles then throws at runtime. | **L** | **High** | L20-08, L23-03, L25-02 | Blocked |
| L28-03 | Add the **kit-side** assertions to `content-scope.test.ts`, deferred from L20-06 because they were false until now: 15 kit ids, `KIT_CATALOG` exhaustive over them, every kit's `specialCards` referencing declared ids, every kit id present in `asset-lookup.ts`'s `KIT_FILES` with a resolvable file. **Acceptance:** the test fails if a 16th kit id is added without art or a catalog entry. | S | Low | L28-02 | To do |
