...
|---|---|---|---|
| **#V4-1** | MEGA ATTACK "attacks every player in the game" — does that include the user? | Sentence says "including the user themselves" explicitly; Poison and Imposition say "all opponents". MEGA says neither. Recommendation: **includes the user**, since the spec had the vocabulary for "opponents" and did not use it. Consequence: a self-targeted pending attack, a case the engine has never resolved (only deferred self-Suicide exists), and `recordEliminationContributor` receiving source = victim. | L23-01 |
...
| **#V4-3** | MEGA ATTACK queues N pending attacks. Which one pairs in a mutual comparison, and what happens to the others? | `findIndex` currently takes the first in array order. Recommendation: **each of the N is evaluated independently on its own target's turn** — what the rules spec says of the Assassin's attacks, so it is consistent. Confirm, then test, including two MEGA ATTACKs facing each other. | L23-01, L20-07 |
| **#V4-4** | Super Mirror: does the user choose which attacks to redirect, or are all of them redirected automatically? | Rules spec: "redirects every attack currently pending against the user toward all opponents". Recommendation: **no choice, all of them, to every opponent** — which removes a sub-choice from §4.4. Confirm, because Mirror *does* have a choice and the asymmetry is easy to misread. | L23-02 |
| **#V4-5** | Attack Thief blocks "once, any attack targeting the user". Is the charge spent on an attack that mutual attacks would have cancelled anyway? | Recommendation: **mutual cancellation runs first, the charge is preserved** — consistent with §5.6's "never silently waste an already-paid card". | L23-03 |
...
| **#V4-31** | Can Attack Thief steal a MEGA ATTACK? | Rules spec: "steals a random attack card from each opponent". After §4.1, MEGA ATTACK *is* an attack card, so the literal reading includes it — and upgraded Attack Thief would take every opponent's MEGA ATTACK at once, for 8 points. Recommendation: **exclude it** (the filter reads `ATTACK_CARD_IDS`, the buyable set), but this is a rule choice, not a typing detail. | L23-03 |
| **#V4-32** | May the Assassin include MEGA ATTACK in a multi-attack? | Rules spec: "can play an unlimited number of attack cards in the same turn, as long as they have the necessary points". Nothing excludes MEGA ATTACK. Recommendation: **exclude it** — it takes no target, so it does not fit `playMultipleAttacks`'s per-slot target payload, and including it would inflate the ≤8 candidate approximation. But it is an exclusion the rules spec does not state. | L23-01 |
