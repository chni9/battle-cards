...
| L53-05 | Felt collapse planner: when dock min-height (64px hand) does not fit, hide Incoming (and Waiting) then opponents then the action log behind a button + Dialog, in that order. Empty Incoming/Waiting take no space even before collapse. **Acceptance:** `planFeltCollapse` tests for the three steps; 6p short felt collapses the log last. | M | Medium | L53-03, L53-04 | Done |
