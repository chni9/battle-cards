...
| L63-06 | `mapActionLogForRecipient` omits `buyPoolCard` `cardId` / `isUpgraded` unless recipient sees that actor’s private info; `PlayingStateView.pool` fogs identity except for the pool-pick chooser; belief does not invent opponent cards from fogged logs or pool diffs. **Acceptance:** view + belief tests; Excel `exportLog` stays full; `pnpm verify` green. | M | **High** | L63-02 | Done |
