...
15. **Feedback + lobby rematch (Lot 57).** Designer 2026-09-14 / 2026-09-15.
    Convert Game over Feedback; Ready + Kick; Play again in the same room;
    join-by-code spectate or claim a disconnected seat. `PROTOCOL_VERSION`
    **31 → 32** then **32 → 33** (exception, same class as L49 / L56).
16. **Designer Classic tweaks (Lot 58).** Designer 2026-09-15. Shop upgrade-point
...
17. **Compact Draw / Unspy dock (Lot 59).** Designer 2026-09-15. Economy-bar Draw
    ...
    bump.
18. **Game over awards (Lot 60).** Designer 2026-09-15. Seat-colored award tiles
...
21. **What’s new + Classic nerfs (Lot 63).** Designer 2026-09-20. Hub What’s
    ...
    pool-buy log fog. Super Absorber package 3 (no snapshot, no ×2).
    `PROTOCOL_VERSION` **35 → 36**. Super Regeneration stays +9/+18.
...
## Lot 53 — Table crowding (designer 2026-08-31)

Client presentation only. Mobile-first. Classic 2–6. No protocol bump, no rule change.
Supersedes L43-04 short-dock shrink and L52-01 opponent wrap.

| ID | Task | Cx | Risk | Depends on | Status |
|---|---|---|---|---|---|
| L53-01 | Lock the layout contract in `decisions.md` + spec v6 §6.2: 64px floor, wrap+vertical scroll, no pager, one-row opponents, collapse Incoming → opponents → action log. **Acceptance:** an agent reading only those files knows L43-04 shrink and L52-01 wrap are superseded. | S | Low | — | Done |
...
(Lot 56 freeze superseded). One protocol bump 33 → 34 (retargeted after Lot 57). Do not edit `heuristic-v4`
weights; freeze fixture refresh is L58-08.

| ID | Task | Cx | Risk | Depends on | Status |
|---|---|---|---|---|---|
| L58-01 | Append dated `[P]` Lot 58 entries to `docs/agent/decisions.md`; Lot 58 section here; technical spec v6 §13 + freeze/protocol exceptions; rules spec (pool buy, PG 3/6, Invisibility pacifist 4/7, Unspy 10); AGENTS.md snapshot. Protocol 33 → 34 exception recorded (same class as L49 / L56 / L57). **Acceptance:** an agent reading only `decisions.md` + this backlog can tell Invisibility is live again under the new text, v34 is the next bump, and L58-02 starts the wire. | S | Medium | — | Done |
| L58-02 | `PROTOCOL_VERSION` **33 → 34** (was 31 → 32 before Lot 57 on main). `buyPoolCard`; `clearSpy` `{ targetPlayerId }`; `PlayingStateView.poolBuyCost`; `PublicPlayerView.spyingOnYou`; reject codes `empty-pool` / `not-spying-you`; `GameState.poolBuyCost` init 1; shared `POOL_BUY_INITIAL_COST` / `CLEAR_SPY_COST`. Types + version until later tasks emit them. **Acceptance:** mismatch path rejects v33 clients; exhaustive reject-code test includes the two new codes. | M | **High** | L58-01 | Done |
...
## Lot 60 — Game over awards gallery (designer 2026-09-15)
...
public recap numbers only — zero rule logic. Retargeted from Lot 59 after the
compact Draw/Unspy dock landed on main as Lot 59.

| ID | Task | Cx | Risk | Depends on | Status |
|---|---|---|---|---|---|
| L60-01 | Append dated `[P]` Lot 60 entries to `docs/agent/decisions.md`; Lot 60 section here; technical spec v6 §2 / §8 / §12 / §13; `AGENTS.md` snapshot. Protocol 34 → 35 exception recorded (same class as L49 / L56 / L57 / L58). **Acceptance:** an agent reading only `decisions.md` + this backlog can tell recap kits and match awards are Lot 60, v35 is the next bump, and L60-02 starts the wire. | S | Medium | — | Done |
| L60-02 | `PROTOCOL_VERSION` **34 → 35**. Extend `GameRecapPlayerView` with `kitId?`, `isBot`, match totals, split buy/sell/special/draw/attack counts, `damageDealt`, `kills`, `thinkTimeMs`. Keep existing play/buy/sell/upgrade counts. **Acceptance:** mismatch path rejects v34 clients; recap type has the new fields; `pnpm verify` green. | M | **High** | L60-01 | Done |
| L60-03 | `Player.matchStats` (lives lost/gained, points spent/gained, upgrade points spent). Increment in `applyDamage` and `applyLifeLoss` separately, `gainLives`, `gainPoints`, and every `turnLedger` spend site including `payCost`. Reanimation keeps matchStats. **Acceptance:** theft does not inflate pointsSpent; Reanimation keeps totals; Invisibility turn counters still ignored by `applyDamage`; `pnpm verify` green. | M | **High** | L60-01 | Done |
...
| L61-04 | `GET /api/admin/games/:id` detail (`finished_games.id`) + `GET /api/admin/kit-stats`. **Acceptance:** detail shows seats/elim/winner; Play again / code reuse does not collapse rows; kit rates respect tutorial filter; `pnpm verify` green. | M | Medium | L61-03 | Done |
...
Hub What’s new (localStorage, compact green New + red unread tick, auto-open
this version, before → after with card art, history) plus Classic nerfs:
Sentence 20 delayed kill (3 later caster turns, caster-only scary banners),
...
| L63-03 | Sentence cost 20; 3 later activator turns (play does not count); fire queues kill; cancel if activator dies first; upgraded never self; invisible excluded; caster chip; table-wide scary banner on play (remaining 3) / later decrements / fire (`Sentence will kill {nickname}!`); no banner on other seats’ turns. **Acceptance:** tests lock remaining=3 after play, play+decrement+fire banners, no banner on a non-caster turn, cancel-on-death, upgraded/invisible/empty fizzle, victim-turn life loss; `pnpm verify` green. | L | **High** | L63-02 | Done |
...
| L63-07 | Shared release-notes catalog; compact green New + red unread tick; auto-open this version (How to play is not a blocker); before → after with named-card art; history; playbook “update the latest entry in the same commit as player-visible work”. **Acceptance:** localStorage key; auto-open when last-seen ≠ lot-63; New green + red badge; items have cardId + before ≠ after for Sentence / Imposition / Super Absorber; no Super Regen / Superpowers; `pnpm verify` green. | L | Medium | L63-01 | Done |
...
| L63-09 | Playbooks (`engine.md`, `protocol.md`, `frontend.md`, `card-handler.md`, `testing.md`, `bots.md`) + post-lot browser gate (green New + auto-open, Sentence caster banners + chip, pool faces). **Acceptance:** playbooks match code; browser gate recorded; `pnpm verify` green. | M | Low | L63-07, L63-08 | Done |
