...
| L61-03 | Shared inbox-auth helper; `GET /api/admin/overview` + `GET /api/admin/games` (filters, pagination). Tests: 404/401/429/503, tutorial excluded by default. **Acceptance:** filters work; auth matches inbox; `pnpm verify` green. | M | **High** | L61-02 | In progress |
