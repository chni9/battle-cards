...
2. Add its static data (name, cost, `buyCost`, `sellYield`, description strings) to
   ...
   `effect`, `upgradeEffect`, and `upgradeAdds` are **player-facing text**, never
   executable data. `upgradeEffect` is the **full** upgraded description.
   `upgradeAdds` is the delta shown on non-upgraded faces (L51-05). Upgraded UI
   shows only `upgradeEffect`.
