...
| `INBOX_PASSWORD` | Shared secret for `GET /api/inbox` and `GET /api/admin/*` (`X-Inbox-Password`). Unset or empty → 404 (do not advertise the inbox/admin). Failed guesses: 10 / 10 min / IP then 429; a correct password still succeeds |
