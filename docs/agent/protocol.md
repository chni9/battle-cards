...
>
> **Status:** current `PROTOCOL_VERSION` is **32** (L58-02 pool buy / Unspy /
> `poolBuyCost` / `spyingOnYou`; L56-03 Mirror redirect fields +
...
| `GameState.poolBuyCost` | **Public** in `PlayingStateView` (PROTOCOL_VERSION 32 / L58-02). Starts at 1; doubles after each successful `buyPoolCard`; never resets |
| Who currently spies the recipient | **Public** as `PublicPlayerView.spyingOnYou` on **living** viewers with a real matrix row (PROTOCOL_VERSION 32). Never on `isYou`. Never inferred from the eliminated-spectator overlay |
...
`buySpecialCard` · `buyPoolCard` (PROTOCOL_VERSION 32 / L58-05 — no payload) ·
`clearSpy` (PROTOCOL_VERSION 32 / L58-07 — `{ targetPlayerId }`) ·
...
PROTOCOL_VERSION 32 (L58-02) adds `buyPoolCard` / `clearSpy`, public `poolBuyCost`,
