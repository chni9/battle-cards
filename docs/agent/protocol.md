...
| Duplicator anticipatory window (`duplicationActive`) | **Spy-gated** (designer 2026-08-06): true for self and Spy recipients; `false` for everyone else. `activateDuplication` action-log rows and live `ACTION_PLAYED` follow the same gate; Excel `exportLog` keeps the full server log |
