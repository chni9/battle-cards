...
| Every action played, **including card identity** | **Public** — purchases, sales, upgrades and draws included. **Exception:** `buyPoolCard` is public only as that action; recovered `cardId` / `isUpgraded` are omitted from per-recipient `actionLog` and live `ACTION_PLAYED` (designer 2026-09-20). Excel `exportLog` keeps them |
...
`thinkTimeMs`). Recap **numbers** are identical for every seated recipient; only fogged
walk-ins omit `kitId`. Think time is a **room** wall-clock map, not `GameState` — missing
